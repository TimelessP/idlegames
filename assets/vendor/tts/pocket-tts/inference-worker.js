...
async function ensureCustomVoiceCached(options = {}) {
    const { force = false, statusText = "Preparing custom voice...", cacheKey = "custom" } = options;
    console.log("[worker] ensureCustomVoiceCached start:", { force, cacheKey, hasEmbedding: !!customVoiceEmbedding });
    if (!customVoiceEmbedding) {
        throw new Error("No custom voice loaded.");
    }

    if (!force && voiceConditioningCache.has(cacheKey)) {
        console.log("[worker] ensureCustomVoiceCached using cache for:", cacheKey);
        return voiceConditioningCache.get(cacheKey);
    }

    postMessage({ type: "status", status: statusText, state: "loading" });
    const conditioned = await buildVoiceConditionedState(customVoiceEmbedding);
    voiceConditioningCache.set(cacheKey, conditioned);
    console.log("[worker] ensureCustomVoiceCached set cache for:", cacheKey, "hasCache:", voiceConditioningCache.has(cacheKey));
    return conditioned;
}

async function loadClonedVoiceEmbedding(voiceName, options = {}) {
    const { force = false } = options;
    console.log("[worker] loadClonedVoiceEmbedding start:", { voiceName, force, hasCache: voiceConditioningCache.has(voiceName) });
    if (!force && (voiceConditioningCache.has(voiceName) || (customVoiceEmbedding && currentVoiceName === voiceName))) {
        if (voiceConditioningCache.has(voiceName)) {
            console.log("[worker] loadClonedVoiceEmbedding returning cached conditioning for:", voiceName);
            return voiceConditioningCache.get(voiceName);
        }
        console.log("[worker] loadClonedVoiceEmbedding ensuring cached for:", voiceName);
        return ensureCustomVoiceCached({ force: false, statusText: `Preparing voice (${voiceName})...`, cacheKey: voiceName });
    }
    const embeddingPath = bundlePath(currentLanguage, "voice-commentator.safetensors");
    console.log("[worker] loadClonedVoiceEmbedding fetching:", embeddingPath);
    const response = await fetch(embeddingPath);
    if (!response.ok) {
        throw new Error(`Missing cloned voice embedding at ${embeddingPath}`);
    }
    const buffer = await response.arrayBuffer();
    const parsed = parseSafetensorsEmbedding(buffer);
    customVoiceEmbedding = parsed;
    currentVoiceName = voiceName;
    console.log("[worker] loadClonedVoiceEmbedding parsed safetensors embedding:", { shape: parsed.shape });
    return ensureCustomVoiceCached({ force: true, statusText: `Preparing voice (${voiceName})...`, cacheKey: voiceName });
}
...
function parseSafetensorsEmbedding(buffer) {
    const view = new DataView(buffer);
    const headerLength = Number(view.getBigUint64(0, true));
    const headerBytes = new Uint8Array(buffer, 8, Number(headerLength));
    const headerText = new TextDecoder().decode(headerBytes);
    const header = JSON.parse(headerText);
    const tensorEntry = header.embedding || Object.values(header)[0];
    if (!tensorEntry) {
        throw new Error("No tensors found in safetensors payload");
    }
    const shape = Array.isArray(tensorEntry.shape) ? tensorEntry.shape : [];
    const dataOffsets = Array.isArray(tensorEntry.data_offsets) ? tensorEntry.data_offsets : null;
    if (!dataOffsets || dataOffsets.length !== 2) {
        throw new Error("Invalid safetensors tensor metadata");
    }
    const dataStart = 8 + Number(headerLength) + ((8 - ((8 + Number(headerLength)) % 8)) % 8);
    const dataLength = dataOffsets[1] - dataOffsets[0];
    const raw = new Uint8Array(buffer, dataStart + dataOffsets[0], dataLength);
    const dtype = tensorEntry.dtype || "F32";
    if (dtype === "F32") {
        return { data: new Float32Array(raw.buffer, raw.byteOffset, raw.byteLength / 4), shape };
    }
    if (dtype === "F64") {
        return { data: new Float32Array(raw.buffer, raw.byteOffset, raw.byteLength / 8), shape };
    }
    ...
    try {
        ...
        if (type === "set_voice") {
            ...
            if (data.voiceName === "custom") {
                ...
                currentVoiceName = "custom";
            } else if (data.voiceName === "cloned-commentator") {
                await loadClonedVoiceEmbedding(data.voiceName, { force: false });
...
async function startGeneration(text, voiceName) {
    ...
    try {
        ...
        if (voiceName === "custom") {
            await ensureCustomVoiceCached({ statusText: "Preparing custom voice...", cacheKey: "custom" });
        } else if (voiceName === "cloned-commentator") {
            await loadClonedVoiceEmbedding(voiceName, { force: false });
        } else {
