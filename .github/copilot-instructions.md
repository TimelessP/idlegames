...
- **Key runtime folders:**
  - `assets/`
    ...
    - `vendor/tts/` – optional offline TTS assets for pages such as `airmail.html`. Generated files like `espeak-loader.js`, `espeak-loader.wasm`, and `espeak-loader.data` live here.
...
- **Other important files:** `manifest.webmanifest`, `package.json` (Node 20+), `feedcycle.*` (largest SPA), `parental.js` (launcher parental controls), `build.sh` (helper wrapper that just runs the npm build pipeline).
...
## Offline TTS build workflow
Use this only when changing the offline speech pipeline or regenerating `espeak-ng` browser assets for pages that rely on `assets/vendor/tts/tts-wrapper.js`.

1. Build the Docker image: `npm run espeak:docker-build`
2. Generate the browser artifacts: `npm run espeak:docker-run`
3. Rebuild the site so the generated assets are copied into `dist/`: `npm run build`

Useful shortcuts:
- `npm run build:espeak` – builds the Docker image and regenerates the TTS artifacts.
- `npm run build:with-espeak` – regenerates TTS artifacts and then rebuilds the site.

Important details:
- The Docker build is not a trivial one-step Emscripten compile. It first builds a native `espeak-ng` binary, then performs the Emscripten build using that native binary so compiled runtime data files are generated correctly.
- The generated `espeak-loader.data` must contain compiled runtime files such as `phontab`, `phondata`, and `phonindex`. If speech falls back or fails at runtime with missing `phontab`, the data package was built incorrectly.
- The build script is `assets/vendor/tts/espeak-build/build.sh`. The container image is defined under `assets/vendor/tts/espeak-build/`.
- Generated outputs are written to `assets/vendor/tts/`. These files are build artifacts and should be treated as generated binaries, not hand-edited source.
- Runtime loading is done by `assets/vendor/tts/tts-wrapper.js`, which is expected to prefer real `espeak-ng` output and avoid hiding loader failures behind toy fallback tones.
...
- If a page depends on offline speech, keep the loader contract stable: `tts-wrapper.js` should expose `window.TTS_WASM.speak(text)` returning PCM + sample rate, and the page should still fail clearly if real loader assets are present but broken.
...
3. **If TTS changed:** Run `npm run build:with-espeak` instead of plain `npm run build`, so regenerated `espeak-loader.*` assets are included in the release.
4. **Push:** Commit *all* changed files and push to `main`. The GitHub Action `build-and-deploy.yml` handles the rest.
