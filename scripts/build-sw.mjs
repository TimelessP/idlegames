...
async function ensureFonts() {
  ...
  async function writeFontFaceSheet({
    packageName,
    family,
    weights,
    filePattern,
    ...
    const sourceDir = path.join(projectRoot, 'node_modules', '@fontsource', packageName, 'files');
    const fontsDir = path.join(cssDir, outputDirName);
    await fs.mkdir(fontsDir, { recursive: true });
    let css = '/* Generated by scripts/build-sw.mjs */\n';
    for (const weight of weights) {
      const fileName = filePattern(weight);
      ...
      css += `  src: url('./${outputDirName}/${fileName}') format('woff2');\n`;
      css += `}\n\n`;
    }
    css += `/* End ${family} font declarations */\n`;
    await fs.writeFile(path.join(cssDir, cssFileName), css, 'utf8');
    if (licenseTarget) {
      await fs.copyFile(
        path.join(projectRoot, 'node_modules', '@fontsource', packageName, 'LICENSE'),
        path.join(cssDir, licenseTarget),
      );
    }
  }
  ...
  await writeFontFaceSheet({
    ...
    filePattern: (weight) => `inter-latin-${weight}-normal.woff2`,
    cssFileName: 'inter.css',
    outputDirName: 'inter',
  });
  ...
  await writeFontFaceSheet({
    ...
    filePattern: (weight) => `caveat-latin-${weight}-normal.woff2`,
    cssFileName: 'caveat.css',
